cc.Class({
    ...
    properties: {
        skyNode: cc.Node, //获取背景结点
        pipeNode: cc.Node, //获取管道结点
        birdNode: cc.Node, //获取小鸟结点
        clickLayerNode: cc.Node, //获取点击结点
        scoreNode: cc.Node, //获取分数结点
        playNode: cc.Node, //开始游戏的图片
        numNode: cc.Node, //加分动画
        gameOverNode: cc.Node, //GAMEOVER图片
        birdSkinNode: {
            // 存放小鸟图片
            default: [],
            type: cc.SpriteFrame
    ...
    showBag() {
        ...
        cc.find("Canvas").addChild(node); //将实例化后的预制节点添加到主画布
        node.opacity = 0; //透明度为0
        node.scale = 0.1; //缩放比例
        node.runAction(cc.spawn(cc.fadeIn(0.5), cc.scaleTo(1, 1)));
        // 动画实现，渐入，缩放
    },
    ...
    onClickStartButton() {
        ...
        this.gameOverNode.active = false; //游戏结束画面影藏
        this.num = 0; //暂存分数
        this.power = 0; //力量
    ...
    addScore() {
        ...
        this.numNode.opacity = 255; //将分数透明度设置为255，用于之后的渐出动画
    ...
    update(dt) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            let w = this.birdNode.width / 2 + child.getChildByName("pipe1").width / 2;
            //钢管与小鸟的最大距离
        ...
        if (checkNode) { //如果钢管有数据，就是开始了x轴的碰撞
            if (checkNode.active) {
                ...
                let birdBottom = this.birdNode.y - this.birdNode.height / 2; //小鸟下部
                let pipeTop = checkNode.y + checkNode.getChildByName("pipe2").y - checkNode.getChildByName("pipe2").height / 2; //钢管上部
                let pipeBottom = checkNode.y - checkNode.getChildByName("pipe2").y - checkNode.getChildByName("pipe2").height / 2; //钢管下部
            ...
            this.checkState = false; //检测状态为false
        }
        if (this.birdNode.y < -cc.winSize.height / 2 || this.birdNode.y > cc.winSize.height / 2) {
            // 上下边界判断
            this.gameOver();
        }
        this.birdNode.angle = this.power * this.speed; //小鸟的仰角
        this.curFrame++; //小鸟的图片下标
        if (this.curFrame > 2) {
            // 小鸟只有3张图
            this.curFrame = 0; //下标置0
        }
        if (this.power > 0) {
            // 当存在向上的力时
            this.birdNode.getComponent(cc.Sprite).spriteFrame = this.birdSkinNode[this.curFrame]; //播放小鸟飞的动画
        }
    },
});